...
* **Random Binary Pairs.** An option for binary molecule dataset where there are multiple entries for each binary pair. Constrains splitting so that the same pairs of SMILES are in the same split with the argument `--split_type random_binary_pairs`. Does not constrain individual molecules to only appear in one or the other, only the specific pairs. Does not depend on the ordering of molecules in the data file.
...
When data contains multiple molecules per datapoint, scaffold and repeated SMILES splitting will only constrain splitting based on one of the molecules. The key molecule can be chosen with the argument `--split_key_molecule <int>`, with the default setting using an index of 0 indicating the first molecule. Random binary pairs splitting does not depend on the key molecule.
