...
def get_initial_train_args(
    ...
    if search_function == "mve":
        ...
        commandline_inputs.extend(["--quantile_loss_alpha", str(active_args.quantile_alpha)])
    elif (
...
def test_predictions(active_args: ActiveArgs, train_args: TrainArgs,gpu,search_function,iteration) -> None:
    ...
    if search_function == "ensemble":
        ...
        argument_input.extend(["--conformal_alpha" , str(active_args.quantile_alpha)])
    elif (
        ...
        for i, line in enumerate(tqdm(reader)):
            for j in active_args.task_names:
                ...
                if search_function == "ensemble":
                    ...
                    whole_data[i].output[
                        j + f"_unc_{active_args.train_sizes[iteration]}"
                    ] = float(line[j + f"_quantile_alpha{active_args.quantile_alpha}_uncal_var"])
...
def get_fingerprint_init(nontest_data:MoleculeDataset,active_args:ActiveArgs,gpu) -> Tuple[MoleculeDataset]:
    ...
    standardized_data = standardized_data[:, ~nan_indices]
    kmeans = KMeans(n_clusters=int(active_args.num_cv_seed*1.5),random_state=0)
    cluster_labels = kmeans.fit_predict(standardized_data)
    cluster_assignments = kmeans.predict(standardized_data)
    ...
    if len(adding_cluster_indices) < active_args.initial_trainval_size:
        ...
        center=np.mean(standardized_data[adding_cluster_indices],axis=0)
        distances = np.linalg.norm(standardized_data[missing_from] - center, axis=1)
        sorted_indices = np.argsort(distances)
        
        adding_cluster_indices = np.concatenate((adding_cluster_indices, sorted_indices[0:count]))
...
def delete_pt_files(path):
    # Walk through all directories and subdirectories
    for root, dirs, files in os.walk(path):
        for file in files:
            # Check if the file has the .pt extension
            if file.endswith(".pt"):
